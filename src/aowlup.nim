...
proc cmdShim(slots: seq[Slot], r: Registry) =
  ...
  for s in slots:
    ...
    # 🔴 NEVER write a shim over the tool it resolves to.
    #
    # A shim is named after the VARIANT, and an installed binary is named after
    # the variant too — so for any component that lives in this very directory
    # (the aowli family is installed here) the two paths are THE SAME FILE. The
    # first version of this wrote the shim on top of `~/.aowl/bin/aowli-interp`,
    # destroying the interpreter and leaving a script whose `which` answer was
    # itself: every `interp` run became infinite self-exec, which presents as a
    # hang burning CPU, not as a missing file. Measured before the guard: a
    # two-line hello world went from 38ms to >4 minutes.
    #
    # Checked by resolved path, not by name, so a variant that merely SHARES a
    # name with something here is still shimmed.
    if res.bin.len > 0 and res.bin == path:
      skipped.add name & " (it IS the installed binary)"
      continue
    if registry.fileExists(path):
      var existing = ""
      try:
        existing = readFile(path)
      except:
        existing = ""
      if not contains(existing, "generated by aowlup"):
        skipped.add name & " (a file that aowlup did not write is already there)"
        continue
    let body = "#!/bin/sh\n" &
      ...
      # ⚠️ `which` exits non-zero for an unresolvable slot, so `|| exit 1` here
      # used to swallow the explanation below and leave the user with a bare
      # exit 1 — the shim's whole job is to be the thing that SAYS the tool is
      # not built. Capture first, then decide.
      "exe=$(" & quoteShell(self) & " which " & s.slot & " 2>/dev/null)\n" &
      "if [ -z \"$exe\" ]; then\n" &
      "  echo \"aowlup: '" & s.slot & "' resolves to nothing — nothing is registered for that\" >&2\n" &
      "  echo \"        slot, or what is registered is not built. Try: aowlup doctor\" >&2\n" &
      "  exit 127\n" &
      "fi\n" &
      "exec \"$exe\" \"$@\"\n"
    try:
  ...
  for s in skipped:
    stdout.writeLine "  " & amber(GWarn) & " " & gray("skipped " & s)
  stdout.writeLine "  " & gray("add to PATH →") & "  " &
